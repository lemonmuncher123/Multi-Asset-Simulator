...
class TestReturnDrivers:
    ...
    def test_zero_cost_basis_pnl_pct_is_none(self, db_conn):
        ...
        # Simulate zero cost basis via a "free shares" buy. Schema v12
        # rejects `price = 0` outright; using `price = NULL` instead lets
        # us model the same edge case (e.g. a stock split or gift) — the
        # SQL `SUM(quantity * price)` ignores NULL terms, so cost_basis
        # collapses to 0.
        db_conn.execute(
            "INSERT INTO transactions (date, txn_type, asset_id, quantity, price, total_amount) "
            "VALUES ('2025-01-15', 'buy', ?, 10, NULL, 0.0)",
            (asset.id,))
        db_conn.commit()
...
class TestRealEstateSnapshot:
    ...
    def test_average_ltv(self, db_conn):
        ...
        ledger.add_mortgage(
            db_conn, property_id=p1.id, original_amount=160000.0,
            interest_rate=0.0, payment_per_period=1000.0,
        )
        _, p2, _ = ledger.add_property(
            db_conn, "2025-02-01", symbol="H2", name="House 2",
        ...
        ledger.add_mortgage(
            db_conn, property_id=p2.id, original_amount=200000.0,
            interest_rate=0.0, payment_per_period=1500.0,
        )
        result = get_real_estate_snapshot(db_conn)
...
class TestDashboardPageSmoke:
    ...
    def test_bankruptcy_trigger_hidden_when_empty(self, qapp, db_conn):
        from src.gui.pages.dashboard import DashboardPage
        page = DashboardPage(db_conn)
        page.refresh()
        assert page.bankruptcy_trigger_frame.isHidden()
        assert page.bankruptcy_trigger_table.rowCount() == 0

    def test_bankruptcy_trigger_lists_active_events(self, qapp, db_conn):
        from src.gui.pages.dashboard import DashboardPage
        ...
        record_bankruptcy_event(
            ...
            amount_due=200.0,
            cash_balance=0.0,
            shortfall_amount=200.0,
            notes="seeded")
        page = DashboardPage(db_conn)
        page.refresh()
        assert not page.bankruptcy_trigger_frame.isHidden()
        assert page.bankruptcy_trigger_table.rowCount() == 1
        assert page.bankruptcy_trigger_table.item(0, 0).text() == "2025-02-01"
        assert page.bankruptcy_trigger_table.item(0, 1).text() == "Debt"
        assert page.bankruptcy_trigger_table.item(0, 2).text() == "Loan"
        assert page.bankruptcy_trigger_table.item(0, 3).text() == "2025-02-01"
...
class TestDashboardCompactDisplay:
    ...
    def test_real_estate_money_compact_with_full_tooltip(self, qapp, db_conn):
        ...
        _, prop, _ = ledger.add_property(
            ...
            down_payment=1_500_000_000.0)
        ledger.add_mortgage(
            db_conn, property_id=prop.id, original_amount=1_000_000_000.0,
            interest_rate=0.0, payment_per_period=5_000_000.0,
        )
        page = DashboardPage(db_conn)
        page.refresh()
