...
def test_transaction_import_rolls_back_on_constraint_failure(db_conn):
    ...
    csv_text = (
        "date,txn_type,asset_symbol,quantity,price,total_amount,currency,fees,notes\n"
        "2025-01-05,deposit_cash,,,,500.0,USD,0,test\n"
        "2025-01-06,buy,GHOST,1,100.0,-100.0,USD,0,\n"
    )
...
# --- CSV import safety: reject transaction types that need sibling-table updates ---
#
# The simple CSV format only carries the 9 transaction columns — there's no
# place for debt schedules, mortgage records, or property metadata. Inserting
# pay_debt / pay_mortgage / add_debt / add_property / sell_property /
# update_property_value rows directly into `transactions` would leave the
# sibling tables (debts, mortgages, properties, *_payment_records) out of
# sync. The CSV importer rejects these types with a clear message pointing
# users to Full Data Import instead.

def test_csv_importable_types_is_proper_subset_of_valid_types():
    from src.engines.import_export import (
        CSV_IMPORTABLE_TXN_TYPES, VALID_TXN_TYPES,
    )
    assert CSV_IMPORTABLE_TXN_TYPES.issubset(VALID_TXN_TYPES)
    assert CSV_IMPORTABLE_TXN_TYPES != VALID_TXN_TYPES


def test_csv_importable_types_includes_simple_cash_movements():
    from src.engines.import_export import CSV_IMPORTABLE_TXN_TYPES
    for tt in ("deposit_cash", "withdraw_cash", "buy", "sell",
               "receive_rent", "pay_property_expense", "manual_adjustment"):
        assert tt in CSV_IMPORTABLE_TXN_TYPES, f"{tt} should be CSV-importable"


def test_csv_importable_types_excludes_sibling_table_types():
    from src.engines.import_export import CSV_IMPORTABLE_TXN_TYPES
    for tt in ("pay_debt", "pay_mortgage", "add_property", "add_debt",
               "sell_property", "update_property_value"):
        assert tt not in CSV_IMPORTABLE_TXN_TYPES, (
            f"{tt} must NOT be CSV-importable — it needs sibling-table writes"
        )


@pytest.mark.parametrize("blocked_type", [
    "pay_debt", "pay_mortgage", "add_property", "add_debt",
    "sell_property", "update_property_value",
])
def test_csv_import_rejects_blocked_type_with_full_data_pointer(db_conn, blocked_type):
    deposit_cash(db_conn, "2025-01-01", 10000.0)
    csv_text = (
        "date,txn_type,asset_symbol,quantity,price,total_amount,currency,fees,notes\n"
        f"2025-02-01,{blocked_type},,,,1.0,USD,0,\n"
    )
    result = import_transactions_csv(db_conn, csv_text)
    assert result.imported == 0
    assert any(blocked_type in e and "Full Data" in e for e in result.errors), (
        f"expected Full Data Import pointer in error, got: {result.errors}"
    )


def test_csv_import_still_accepts_safe_types(db_conn):
    """Regression guard: the new restriction must not block the types
    that have always been safely CSV-importable."""
    create_asset(db_conn, Asset(symbol="AAPL", name="Apple", asset_type="stock"))
    deposit_cash(db_conn, "2025-01-01", 100000.0)
    csv_text = (
        "date,txn_type,asset_symbol,quantity,price,total_amount,currency,fees,notes\n"
        "2025-02-01,deposit_cash,,,,500.0,USD,0,\n"
        "2025-02-02,withdraw_cash,,,,-200.0,USD,0,\n"
        "2025-02-03,buy,AAPL,5,150.0,-750.0,USD,0,\n"
        "2025-02-04,sell,AAPL,2,160.0,320.0,USD,0,\n"
        "2025-02-05,manual_adjustment,,,,-10.0,USD,0,\n"
    )
    result = import_transactions_csv(db_conn, csv_text)
    assert result.imported == 5, f"errors: {result.errors}"


# --- Sign-of-total_amount regressions (BUG-1) ---


@pytest.mark.parametrize("txn_type,bad_amount", [
    ("buy", "1500.0"),       # buy must be negative
    ("sell", "-320.0"),      # sell must be positive
    ("deposit_cash", "-50.0"),
    ("withdraw_cash", "50.0"),
    ("receive_rent", "-100.0"),
    ("pay_property_expense", "100.0"),
])
def test_csv_import_rejects_wrong_total_amount_sign(db_conn, txn_type, bad_amount):
    """A buy must have a negative total_amount, a sell positive, etc.
    Without this guard the importer would silently inflate cash and
    position simultaneously, breaking the source-of-truth invariant.
    """
    create_asset(db_conn, Asset(symbol="AAPL", name="Apple", asset_type="stock"))
    deposit_cash(db_conn, "2025-01-01", 100000.0)
    asset_col = "AAPL" if txn_type in ("buy", "sell", "receive_rent", "pay_property_expense") else ""
    qty = "5" if txn_type in ("buy", "sell") else ""
    price = "150.0" if txn_type in ("buy", "sell") else ""
    csv_text = (
        "date,txn_type,asset_symbol,quantity,price,total_amount,currency,fees,notes\n"
        f"2025-02-03,{txn_type},{asset_col},{qty},{price},{bad_amount},USD,0,\n"
    )
    result = import_transactions_csv(db_conn, csv_text)
    assert result.imported == 0, f"expected rejection, got imported={result.imported}"
    assert any("must be" in e for e in result.errors)


def test_csv_import_allows_manual_adjustment_either_sign(db_conn):
    """manual_adjustment is the documented escape hatch — both signs OK."""
    deposit_cash(db_conn, "2025-01-01", 1000.0)
    csv_text = (
        "date,txn_type,asset_symbol,quantity,price,total_amount,currency,fees,notes\n"
        "2025-02-01,manual_adjustment,,,,500.0,USD,0,inflow\n"
        "2025-02-02,manual_adjustment,,,,-100.0,USD,0,outflow\n"
    )
    result = import_transactions_csv(db_conn, csv_text)
    assert result.imported == 2, f"errors: {result.errors}"


# --- buy/sell shape regressions (BUG-2) ---


def test_csv_import_rejects_buy_without_quantity(db_conn):
    create_asset(db_conn, Asset(symbol="AAPL", name="Apple", asset_type="stock"))
    deposit_cash(db_conn, "2025-01-01", 100000.0)
    csv_text = (
        "date,txn_type,asset_symbol,quantity,price,total_amount,currency,fees,notes\n"
        "2025-02-01,buy,AAPL,,150,-1500,USD,0,\n"
    )
    result = import_transactions_csv(db_conn, csv_text)
    assert result.imported == 0
...
def test_csv_import_rejects_buy_without_price(db_conn):
    create_asset(db_conn, Asset(symbol="AAPL", name="Apple", asset_type="stock"))
    deposit_cash(db_conn, "2025-01-01", 100000.0)
    csv_text = (
        "date,txn_type,asset_symbol,quantity,price,total_amount,currency,fees,notes\n"
        "2025-02-01,buy,AAPL,10,,-1500,USD,0,\n"
    )
    result = import_transactions_csv(db_conn, csv_text)
    assert result.imported == 0
...
def test_csv_import_rejects_buy_with_zero_quantity(db_conn):
    create_asset(db_conn, Asset(symbol="AAPL", name="Apple", asset_type="stock"))
    deposit_cash(db_conn, "2025-01-01", 100000.0)
    csv_text = (
        "date,txn_type,asset_symbol,quantity,price,total_amount,currency,fees,notes\n"
        "2025-02-01,buy,AAPL,0,150,-1500,USD,0,\n"
    )
    result = import_transactions_csv(db_conn, csv_text)
    assert result.imported == 0


# --- manual_adjustment quantity rules (BUG-2) ---


def test_csv_import_manual_adjustment_with_qty_requires_price(db_conn):
    """ledger.manual_adjustment refuses qty without price; CSV must too."""
    create_asset(db_conn, Asset(symbol="AAPL", name="Apple", asset_type="stock"))
    deposit_cash(db_conn, "2025-01-01", 1000.0)
    csv_text = (
        "date,txn_type,asset_symbol,quantity,price,total_amount,currency,fees,notes\n"
        "2025-02-01,manual_adjustment,AAPL,5,,0,USD,0,\n"
    )
    result = import_transactions_csv(db_conn, csv_text)
    assert result.imported == 0
    assert any("positive price" in e for e in result.errors)
...
def test_csv_import_manual_adjustment_with_qty_requires_sellable_type(db_conn):
    """Cannot adjust quantity for non-sellable types (real_estate, debt)."""
    create_asset(db_conn, Asset(symbol="HOUSE", name="House", asset_type="real_estate"))
    deposit_cash(db_conn, "2025-01-01", 1000.0)
    csv_text = (
        "date,txn_type,asset_symbol,quantity,price,total_amount,currency,fees,notes\n"
        "2025-02-01,manual_adjustment,HOUSE,1,500000,0,USD,0,\n"
    )
    result = import_transactions_csv(db_conn, csv_text)
    assert result.imported == 0
    assert any("cannot change quantity" in e for e in result.errors)


def test_csv_import_manual_adjustment_with_qty_and_price_succeeds(db_conn):
    create_asset(db_conn, Asset(symbol="AAPL", name="Apple", asset_type="stock"))
    deposit_cash(db_conn, "2025-01-01", 1000.0)
    csv_text = (
        "date,txn_type,asset_symbol,quantity,price,total_amount,currency,fees,notes\n"
        "2025-02-01,manual_adjustment,AAPL,5,150.0,0,USD,0,corrective\n"
    )
    result = import_transactions_csv(db_conn, csv_text)
    assert result.imported == 1, f"errors: {result.errors}"
