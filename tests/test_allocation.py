...
def test_debt_ratio_with_additional_debt(portfolio):
    ledger.add_debt(portfolio, "2025-01-01", symbol="CC", name="Card", amount=10000.0, cash_received=False)
...
def test_real_estate_falls_back_to_symbol(db_conn):
    ledger.deposit_cash(db_conn, "2025-01-01", 200000.0)
    ledger.add_property(
        db_conn, "2025-02-01", symbol="RE_HOME", name="",
        purchase_price=500000.0, mortgage_balance=400000.0,
    )
    items = calc_allocation_by_asset(db_conn)
    re_items = [i for i in items if i["asset_type"] == "real_estate"]
    assert len(re_items) == 1
    assert re_items[0]["name"] == "RE_HOME"
