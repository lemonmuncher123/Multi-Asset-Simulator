...
class TestQuarterlyAndSemiAnnualReports:
    def test_generate_quarterly_report_q1(self, db_conn):
        from src.engines.ledger import deposit_cash
        from src.engines.reports import generate_quarterly_report

        deposit_cash(db_conn, "2026-01-15", 100.0)
        deposit_cash(db_conn, "2026-02-20", 200.0)
        deposit_cash(db_conn, "2026-04-05", 50.0)  # outside Q1
        ...
        assert r.title == "Quarterly Report - 2026-Q1"

        data = json.loads(r.report_json)
        assert data["summary"]["transaction_count"] == 2
        assert data["summary"]["net_cash_flow"] == 300.0

    def test_generate_quarterly_report_q4_rolls_year(self, db_conn):
        from src.engines.ledger import deposit_cash
        from src.engines.reports import generate_quarterly_report

        deposit_cash(db_conn, "2026-11-10", 100.0)
        r = generate_quarterly_report(db_conn, 2026, 4)
        assert r.period_start == "2026-10-01"
        assert r.period_end == "2027-01-01"

    def test_generate_semi_annual_report_h2(self, db_conn):
        from src.engines.ledger import deposit_cash
        from src.engines.reports import generate_semi_annual_report

        deposit_cash(db_conn, "2026-08-15", 100.0)
        ...
        assert r.period_end == "2027-01-01"
        assert r.title == "Semi-Annual Report - 2026-H2"

    def test_generate_semi_annual_report_h1(self, db_conn):
        from src.engines.ledger import deposit_cash
        from src.engines.reports import generate_semi_annual_report

        deposit_cash(db_conn, "2026-03-10", 100.0)
        deposit_cash(db_conn, "2026-08-10", 200.0)  # outside H1
        r = generate_semi_annual_report(db_conn, 2026, 1)
        assert r.period_start == "2026-01-01"
        assert r.period_end == "2026-07-01"
        data = json.loads(r.report_json)
        assert data["summary"]["transaction_count"] == 1
        assert data["summary"]["net_cash_flow"] == 100.0

    def test_generate_due_reports_includes_quarters_and_halves(self, db_conn):
        """earliest txn 2025-03-15, today 2026-08-01 → should generate
        all quarters/halves whose end is <= today, plus the existing
        monthly + 2025 annual reports."""
        from src.engines.ledger import deposit_cash
        from src.engines.reports import generate_due_reports

        deposit_cash(db_conn, "2025-03-15", 1.0)
        ...
        for q in ("2025-Q1", "2025-Q2", "2025-Q3", "2025-Q4",
                  "2026-Q1", "2026-Q2"):
            assert ("quarterly", q) in types, f"missing quarterly {q}"
        # 2026-Q3 ends 2026-10-01 > today → not generated
        assert ("quarterly", "2026-Q3") not in types
        for h in ("2025-H1", "2025-H2", "2026-H1"):
            assert ("semi_annual", h) in types, f"missing semi_annual {h}"
        assert ("semi_annual", "2026-H2") not in types

    def test_count_due_reports_includes_new_types(self, db_conn):
        from src.engines.ledger import deposit_cash
        from src.engines.reports import count_due_reports

        deposit_cash(db_conn, "2026-01-15", 1.0)
        # today=2026-08-01:
        # monthly: Jan, Feb, Mar, Apr, May, Jun, Jul = 7
        # quarterly: Q1 (ends Apr-1), Q2 (ends Jul-1) = 2
        # semi_annual: H1 (ends Jul-1) = 1
        # annual: none (current year, end > today)
        n = count_due_reports(db_conn, today=date(2026, 8, 1))
        assert n == 7 + 2 + 1

    def test_generate_due_reports_idempotent(self, db_conn):
        """Re-running generate_due_reports after some are present yields
        only the missing ones, none of the existing types regenerated."""
        from src.engines.ledger import deposit_cash
        from src.engines.reports import (
            generate_due_reports, generate_quarterly_report,
        )

        deposit_cash(db_conn, "2025-03-15", 1.0)
        generate_quarterly_report(db_conn, 2025, 1)
        generated = generate_due_reports(db_conn, today=date(2025, 12, 1))
        types = {(r.report_type, r.period_label) for r in generated}
        # 2025-Q1 was pre-generated → must not be in the new generation set
        assert ("quarterly", "2025-Q1") not in types
        # 2025-Q2, Q3 ended before 2025-12-01 → should be generated now
        assert ("quarterly", "2025-Q2") in types
        assert ("quarterly", "2025-Q3") in types


class TestReportStatsAllFourTypes:
    def test_get_report_stats_includes_quarterly_and_semi_annual(self, db_conn):
        from src.engines.ledger import deposit_cash
        from src.engines.reports import (
        ...
        deposit_cash(db_conn, "2025-01-15", 1.0)
    ...
    def test_get_report_stats_zero_when_empty(self, db_conn):
        from src.storage.report_repo import get_report_stats
        stats = get_report_stats(db_conn)
        assert stats == {
...
class TestFeesBreakdownSection:
    def test_build_period_report_includes_fees_breakdown(self, db_conn):
        """Buy + sell with broker_commission + sec_section31 → grouped
        ...
        deposit_cash(db_conn, "2026-03-01", 10000.0)
        buy_txn = buy(db_conn, "2026-03-10", a.id, 10, 150.0, fees=2.5)
        create_fee_breakdown(db_conn, FeeBreakdownRow(
            transaction_id=buy_txn.id, fee_type="broker_commission",
            amount=2.5,
        ))
        sell_txn = sell(db_conn, "2026-03-20", a.id, 5, 160.0, fees=3.0)
        create_fee_breakdown(db_conn, FeeBreakdownRow(
            transaction_id=sell_txn.id, fee_type="broker_commission",
            amount=2.5,
        ))
        create_fee_breakdown(db_conn, FeeBreakdownRow(
            transaction_id=sell_txn.id, fee_type="sec_section31",
            amount=0.5,
        ))
        ...
        # ORDER BY total DESC → broker_commission must come first
        assert fb["by_type"][0]["fee_type"] == "broker_commission"

    def test_build_period_report_fees_breakdown_empty(self, db_conn):
        """No fee_breakdown rows in period → present-but-empty section."""
        from src.engines.ledger import deposit_cash
        from src.engines.reports import generate_monthly_report

        deposit_cash(db_conn, "2026-03-01", 100.0)
        r = generate_monthly_report(db_conn, 2026, 3)
        data = json.loads(r.report_json)
        assert data["fees_breakdown"] == {
            "by_type": [], "grand_total": 0.0,
        }

    def test_approximate_return_pct_skipped_when_beginning_nw_below_threshold(self, db_conn):
        """Tiny beginning net worth (< $100) → return_pct=None with an
        explanatory data-quality note, even when end is large."""
        from datetime import date as _date
        from src.engines.ledger import deposit_cash
        from src.engines.snapshots import record_daily_portfolio_snapshot
        from src.engines.reports import generate_monthly_report

        deposit_cash(db_conn, "2025-12-15", 5.0)  # tiny start
        record_daily_portfolio_snapshot(db_conn, snapshot_date=_date(2026, 3, 1))
        deposit_cash(db_conn, "2026-03-15", 10_000.0)
        record_daily_portfolio_snapshot(db_conn, snapshot_date=_date(2026, 4, 1))
        ...
        assert perf["approximate_return_pct"] is None
        note = perf["data_quality_note"].lower()
        assert "below" in note or "small" in note

    def test_approximate_return_pct_computed_when_above_threshold(self, db_conn):
        from datetime import date as _date
        from src.engines.ledger import deposit_cash
        from src.engines.snapshots import record_daily_portfolio_snapshot
        from src.engines.reports import generate_monthly_report

        deposit_cash(db_conn, "2025-12-15", 1000.0)
        record_daily_portfolio_snapshot(db_conn, snapshot_date=_date(2026, 3, 1))
        record_daily_portfolio_snapshot(db_conn, snapshot_date=_date(2026, 4, 1))

        r = generate_monthly_report(db_conn, 2026, 3)
        data = json.loads(r.report_json)
        # No new transactions in March, beginning_nw ≥ $100 → return_pct
        # should be computed (approximately 0%, but not None).
        assert data["performance"]["approximate_return_pct"] is not None

    def test_build_period_report_fees_breakdown_excludes_other_periods(self, db_conn):
        """Fee rows on transactions outside the period must not leak in."""
        ...
        deposit_cash(db_conn, "2026-02-01", 10000.0)
        feb_buy = buy(db_conn, "2026-02-15", a.id, 1, 150.0, fees=1.0)
        create_fee_breakdown(db_conn, FeeBreakdownRow(
            transaction_id=feb_buy.id, fee_type="broker_commission",
            amount=1.0,
        ))
        mar_buy = buy(db_conn, "2026-03-15", a.id, 1, 150.0, fees=2.0)
        create_fee_breakdown(db_conn, FeeBreakdownRow(
            transaction_id=mar_buy.id, fee_type="broker_commission",
            amount=2.0,
        ))

        r = generate_monthly_report(db_conn, 2026, 3)
        data = json.loads(r.report_json)
        # Only March's fee should appear.
        assert data["fees_breakdown"]["grand_total"] == 2.0
        assert data["fees_breakdown"]["by_type"][0]["count"] == 1


# ===================================================================
# Period-boundary regressions
#   P1: ending snapshot must not pull from the next period
#   P2: quarterly/semi_annual auto-backfill must start from the first
#       quarter/half containing earliest, not from Q1/H1
#   P3: delete_reports_before_date must use period_end (exclusive) <=
#       cutoff so the period containing the first transaction survives
# ===================================================================


class TestPeriodBoundaryRegressions:

    # --- P1 ---

    def test_ending_snapshot_excludes_period_end_date(self, db_conn):
        """For April 2026 (period_end exclusive = 2026-05-01), a snapshot
        dated exactly on 2026-05-01 must NOT be picked as the ending
        snapshot — that snapshot belongs to May, not April. The correct
        ending snapshot is the most recent on or before 2026-04-30."""
        from datetime import date as _date_cls
        from src.engines.ledger import deposit_cash
        from src.engines.snapshots import record_daily_portfolio_snapshot
        from src.engines.reports import generate_monthly_report

        deposit_cash(db_conn, "2026-04-15", 1000.0)
        record_daily_portfolio_snapshot(db_conn, snapshot_date=_date_cls(2026, 4, 1))
        record_daily_portfolio_snapshot(db_conn, snapshot_date=_date_cls(2026, 4, 30))
        record_daily_portfolio_snapshot(db_conn, snapshot_date=_date_cls(2026, 5, 1))

        r = generate_monthly_report(db_conn, 2026, 4)
        data = json.loads(r.report_json)

        # Beginning is correctly 2026-04-01 (on-or-before period_start).
        assert data["beginning_snapshot"]["snapshot_date"] == "2026-04-01"
        # Ending must be 2026-04-30, NOT 2026-05-01.
        assert data["ending_snapshot"]["snapshot_date"] == "2026-04-30", (
            "Ending snapshot leaked from the next period (2026-05-01); "
            "snapshot_date <= end_date used the exclusive boundary."
        )

    def test_ending_snapshot_uses_april_30_when_no_april_30_snapshot(self, db_conn):
        """If only 2026-04-15 and 2026-05-01 snapshots exist, ending
        should be 2026-04-15 — the latest on or before 2026-04-30 — and
        NOT 2026-05-01."""
        from datetime import date as _date_cls
        from src.engines.ledger import deposit_cash
        from src.engines.snapshots import record_daily_portfolio_snapshot
        ...
        deposit_cash(db_conn, "2026-04-10", 1000.0)
        record_daily_portfolio_snapshot(db_conn, snapshot_date=_date_cls(2026, 4, 15))
        record_daily_portfolio_snapshot(db_conn, snapshot_date=_date_cls(2026, 5, 1))

        r = generate_monthly_report(db_conn, 2026, 4)
        data = json.loads(r.report_json)
        assert data["ending_snapshot"]["snapshot_date"] == "2026-04-15"

    # --- P2 ---

    def test_generate_due_reports_quarterly_skips_pre_first_txn_quarters(self, db_conn):
        """First txn 2026-11-15. Today 2027-02-01. Quarterly should NOT
        generate Q1/Q2/Q3 of 2026 (no transactions in those periods).
        Must generate 2026-Q4 (contains the first txn)."""
        from src.engines.ledger import deposit_cash
        from src.engines.reports import generate_due_reports

        deposit_cash(db_conn, "2026-11-15", 1000.0)
        generated = generate_due_reports(db_conn, today=date(2027, 2, 1))

        labels = {(r.report_type, r.period_label) for r in generated}
        assert ("quarterly", "2026-Q1") not in labels
        assert ("quarterly", "2026-Q2") not in labels
        assert ("quarterly", "2026-Q3") not in labels
        assert ("quarterly", "2026-Q4") in labels

    def test_generate_due_reports_semi_annual_skips_pre_first_txn_halves(self, db_conn):
        """First txn 2026-11-15 (in H2). Today 2027-02-01. Semi-annual
        should NOT generate 2026-H1; must generate 2026-H2."""
        from src.engines.ledger import deposit_cash
        from src.engines.reports import generate_due_reports
        ...
        labels = {(r.report_type, r.period_label) for r in generated}
        assert ("semi_annual", "2026-H1") not in labels
        assert ("semi_annual", "2026-H2") in labels

    def test_count_due_reports_skips_pre_first_txn_quarters_and_halves(self, db_conn):
        """count_due_reports must mirror generate_due_reports — no empty
        pre-first-txn quarter/half counted."""
        from src.engines.ledger import deposit_cash
        from src.engines.reports import count_due_reports

        deposit_cash(db_conn, "2026-11-15", 1000.0)
        # today=2027-02-01:
        # monthly: Nov, Dec, Jan = 3
        # quarterly: 2026-Q4 = 1
        # semi_annual: 2026-H2 = 1
        # annual: 2026 = 1
        assert count_due_reports(db_conn, today=date(2027, 2, 1)) == 3 + 1 + 1 + 1

    def test_generate_due_reports_quarterly_starts_at_correct_quarter(self, db_conn):
        """Earliest in Q3 (Aug 2026). Q1, Q2 must not be generated; Q3
        and Q4 must be."""
        from src.engines.ledger import deposit_cash
        from src.engines.reports import generate_due_reports

        deposit_cash(db_conn, "2026-08-10", 1000.0)
        generated = generate_due_reports(db_conn, today=date(2027, 2, 1))
        labels = {(r.report_type, r.period_label) for r in generated}
        assert ("quarterly", "2026-Q1") not in labels
        assert ("quarterly", "2026-Q2") not in labels
        assert ("quarterly", "2026-Q3") in labels
        assert ("quarterly", "2026-Q4") in labels

    # --- P3 ---

    def test_delete_reports_before_date_keeps_period_containing_cutoff(self, db_conn):
        """Cutoff = first-txn date 2026-11-15. The November monthly
        (period 2026-11-01 — 2026-12-01) contains the first transaction
        and MUST survive. Older empty reports must be deleted."""
        from src.storage.report_repo import (
            delete_reports_before_date, report_exists,
        )

        # Insert 4 monthly reports manually so the test doesn't depend on
        # generate_due_reports behavior.
        def _insert(rt, ps, pe, label):
            db_conn.execute(
                "INSERT INTO reports (report_type, period_start, period_end, "
                "period_label, generated_at, title, report_json, notes) "
                "VALUES (?, ?, ?, ?, '2026-12-01T00:00:00', ?, '{}', NULL)",
                (rt, ps, pe, label, label),
            )
            db_conn.commit()

        _insert("monthly", "2026-09-01", "2026-10-01", "2026-09")
        _insert("monthly", "2026-10-01", "2026-11-01", "2026-10")
        _insert("monthly", "2026-11-01", "2026-12-01", "2026-11")  # contains 2026-11-15
        _insert("quarterly", "2026-10-01", "2027-01-01", "2026-Q4")
        _insert("semi_annual", "2026-07-01", "2027-01-01", "2026-H2")
        _insert("annual", "2026-01-01", "2027-01-01", "2026")

        delete_reports_before_date(db_conn, "2026-11-15")

        # Reports whose period ends ON OR BEFORE the cutoff are gone.
        assert not report_exists(db_conn, "monthly", "2026-09")
        assert not report_exists(db_conn, "monthly", "2026-10")
        # Reports whose period extends past the cutoff are kept — they
        # cover (in part) the first-transaction date.
        assert report_exists(db_conn, "monthly", "2026-11"), (
            "November monthly was deleted — but period_end=2026-12-01 is "
            "after the cutoff, and the report covers the first txn day."
        )
        assert report_exists(db_conn, "quarterly", "2026-Q4")
        assert report_exists(db_conn, "semi_annual", "2026-H2")
        assert report_exists(db_conn, "annual", "2026")
