...
def test_leverage_custom_threshold(db_conn):
    ...
    # 450k mortgage on a 500k property → debt ratio well over the 40% cap.
    ledger.add_mortgage(
...
# --- default debt annual rate ---

def test_default_debt_annual_rate_pct_default(db_conn):
    from src.storage.settings_repo import (
        get_default_debt_annual_rate_pct, DEFAULT_DEBT_ANNUAL_RATE_PCT)
    assert get_default_debt_annual_rate_pct(db_conn) == DEFAULT_DEBT_ANNUAL_RATE_PCT


def test_default_debt_annual_rate_pct_roundtrip(db_conn):
    from src.storage.settings_repo import (
        get_default_debt_annual_rate_pct, set_default_debt_annual_rate_pct)
    set_default_debt_annual_rate_pct(db_conn, 9.5)
    assert get_default_debt_annual_rate_pct(db_conn) == pytest.approx(9.5)


def test_default_debt_annual_rate_pct_rejects_negative(db_conn):
    from src.storage.settings_repo import set_default_debt_annual_rate_pct
    with pytest.raises(ValueError):
        set_default_debt_annual_rate_pct(db_conn, -1.0)
