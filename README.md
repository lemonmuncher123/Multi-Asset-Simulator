...
### Reports
- Auto-generated reports for every completed period on startup, across **four cadences**: monthly (`YYYY-MM`), quarterly (`YYYY-Q1`..`Q4`), semi-annual (`YYYY-H1`/`H2`), and annual (`YYYY`). Backfill starts from the period that contains the first meaningful transaction, so a Nov 15 first deposit does not produce empty Q1/Q2/Q3 or H1 reports.
- Sections: summary, cash-flow breakdown (funding / trade / real-estate / debt / fees / other), approximate performance (snapshot-based, period-end snapshot informed), allocation, risk summary, **beginning + ending snapshots**, **fees breakdown grouped by `fee_type`** (broker commission, SEC §31, FINRA TAF, …), operations, transactions, trades, real estate, debt, journal, current snapshot.
- The Reports tab in Import / Export italicizes any row whose stored transaction count has drifted from the live count in the period (so backdated edits are visible at a glance), confirms before overwriting an existing report, and exposes a **"How to read this report"** button that surfaces the same preface the exports prepend.
- A typed period picker (year spinner + month / quarter / half selector) replaces free-text input for "Generate Selected Period".
- Export selected report to **`.xlsx`** (multi-sheet workbook) or **`.txt`** (plain text with the "How To Read" preface).
- `approximate_return_pct` is reported as N/A when beginning net worth is below $100 — a tiny starter balance combined with normal cash movement otherwise produces a meaninglessly large percentage.
...
5. Generate any missing reports for completed periods across all four cadences (monthly, quarterly, semi-annual, annual).
...
│   │   ├── reports.py              # Monthly / quarterly / semi-annual / annual report builder
...
Four cadences are produced from the same `build_period_report` pipeline:

| Cadence | Period label | Period range |
|---|---|---|
| Monthly | `2026-04` | calendar month |
| Quarterly | `2026-Q1` … `2026-Q4` | calendar quarter |
| Semi-Annual | `2026-H1` / `2026-H2` | calendar half-year |
| Annual | `2026` | calendar year |

Each report includes the following sections: summary, cash-flow breakdown, performance (approximate), allocation, risk summary, **beginning + ending snapshots**, **fees breakdown** (grouped by `fee_type`), operations, transactions, trades, real estate, debt, journal, current snapshot.

**Auto-generation.** On startup, in response to data changes, and on a 30-minute day-boundary timer, any missing report for a completed period is generated. Backfill for each cadence starts at the period that contains the first meaningful transaction — generating empty reports for periods before the first transaction is explicitly avoided.

**Period boundaries.** All periods use exclusive-end storage (`period_end` is the first day of the next period). User-facing surfaces render the inclusive end (`2026-04-01 to 2026-04-30`) so the dates read intuitively. The ending snapshot is looked up against the inclusive last day of the period, so a snapshot dated on the period-end day (which belongs to the next period) is correctly excluded.

**Reports tab UI.** Lists by cadence; italicizes the period column and shows a tooltip when the stored transaction count has drifted from the live count for the period (so a backdated edit becomes visible). "Generate Selected Period" uses a typed picker dialog (year spinner + month / quarter / half selector) and confirms before overwriting an existing report. A **"How to read this report"** button next to the cadence dropdown opens the same explanatory text that exports prepend.

**Delete Reports Before Auto Start.** Removes only reports whose period ended on or before the first-transaction date (`period_end <= cutoff`). Reports whose period extends past that day — including the period that contains the very first transaction — are kept.
...
> - **`Approximate Return %` is suppressed when beginning net worth is below $100** to avoid magnifying noise from a small starter balance.
...
Export to `.xlsx` (multi-sheet workbook — adds dedicated "Snapshots" and "Fees Breakdown" sheets in addition to the per-section sheets) or `.txt` (with the same "How To Read" text up top; the FEES BREAKDOWN section is omitted when no fees were paid in the period).
