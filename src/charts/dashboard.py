...
def create_net_worth_trend_figure(
    ...
    ax.fill_between(range(len(dates)), assets, alpha=0.08, color=_ASSETS_COLOR)
    ax.plot(range(len(dates)), assets, color=_ASSETS_COLOR,
            linewidth=1, alpha=0.4, label="Assets")
    if any(v > 0 for v in liabilities):
        ax.plot(range(len(dates)), liabilities, color=_LIABILITIES_COLOR,
                linewidth=1, alpha=0.4, label="Liabilities")
    ...
    legend = ax.legend(fontsize=7 if compact else 8, frameon=False, loc="upper left")
