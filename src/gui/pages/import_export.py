...
from PySide6.QtWidgets import (
    ...
    QInputDialog, QListWidget, QTabWidget, QComboBox, QHeaderView, QDialog,
...
from src.storage.report_repo import (
    ...
    report_exists, count_transactions_in_period,
)
...
from src.engines.report_export import (
    export_report_txt, export_report_xlsx, HOW_TO_READ,
)
from src.utils.display import (
    money_or_na as _money_or_na,
    percent_or_na as _pct_or_na,
    fraction_as_percent_or_na as _alloc_pct_or_na,
...
class ImportExportPage(QWidget):
    ...
    def __init__(self, conn: sqlite3.Connection, parent=None):
        ...
        type_row.addWidget(btn_how_to_read)
    ...
    def _refresh_report_list(self, *_args):
        ...
        with QSignalBlocker(self.report_list_table):
            ...
            for i, s in enumerate(summaries):
                ...
                live_count = count_transactions_in_period(
                    self.conn, s.period_start, s.period_end,
                )
                if live_count != s.transaction_count:
                    period_item.setFont(italic_font)
                    period_item.setToolTip(
                        f"Stale: report has {s.transaction_count} txns, "
                        f"period now has {live_count}. Use 'Generate "
                        f"Selected Period' to refresh."
                    )
    ...
    def _render_summary_section(self, data: dict):
        s = data.get("summary", {})
        summary_rows = [
            ("Report Type", str(s.get("report_type", ""))),
            ("Period", format_period_inclusive(
    ...
    def _render_performance_section(self, data: dict):
    ...
    def _render_cash_flow_breakdown_section(self, data: dict):
    ...
    def _render_allocation_section(self, data: dict):
    ...
    def _render_snapshots_section(self, data: dict):
        ...
        self.report_snapshots_table.setRowCount(len(snapshots_rows))
        for i, (metric, value) in enumerate(snapshots_rows):
    ...
    def _render_fees_breakdown_section(self, data: dict):
    ...
    def _render_risk_summary_section(self, data: dict):
    ...
    def _render_operations_section(self, data: dict):
    ...
    def _render_transactions_section(self, data: dict):
        ...
        for i, t in enumerate(txns):
            ...
            price = t.get("price")
            self.report_txns_table.setItem(i, 4, QTableWidgetItem(f"${price:,.2f}" if price is not None else ""))
    ...
    def _render_trades_section(self, data: dict):
        ...
        for i, t in enumerate(trades):
            ...
            price = t.get("price")
            self.report_trades_table.setItem(i, 4, QTableWidgetItem(f"${price:,.2f}" if price is not None else ""))
    ...
    def _render_real_estate_section(self, data: dict):
    ...
    def _render_debt_section(self, data: dict):
    ...
    def _render_journal_section(self, data: dict):
    ...
    def _render_current_snapshot_section(self, data: dict):
    ...
    def _generate_selected_period(self):
        ...
        normalized = label_for_type[rtype]

        if report_exists(self.conn, rtype, normalized):
            if not self._confirm_overwrite(normalized):
                return
        ...
        self._refresh_report_list()
        QMessageBox.information(
            self, "Reports", f"Generated {rtype} report for {normalized}.",
        )
    ...
    def _delete_old_reports(self):
        ...
        reply = QMessageBox.question(
            self, "Delete Old Reports",
            f"Delete all reports whose period ended on or before {cutoff}?\n\n"
            f"This cutoff is the date of the earliest meaningful transaction.\n"
            f"Reports whose period extends past that date — including the "
            f"period that contains the first transaction — are kept.\n"
    ...
    def _clear_report_detail(self):
        for table in (
            ...
            self.report_snapshots_table,
            self.report_fees_table,
    ...
    def _show_how_to_read(self):
        # `QMessageBox.information` lays out the full text inline and
        # grows to fit, so HOW_TO_READ (~85 lines) overflows the screen
        # with the OK button pushed off the bottom — the dialog can't
        # be closed normally. Use a sized, scrollable QDialog instead.
        dlg = QDialog(self)
        dlg.setWindowTitle("How to read this report")
        dlg.resize(720, 560)
        ...
        text = QPlainTextEdit(HOW_TO_READ, dlg)
        ...
        # The Close button maps to RejectRole by default — wire it
        # explicitly so Esc and the X also dismiss the dialog cleanly.
