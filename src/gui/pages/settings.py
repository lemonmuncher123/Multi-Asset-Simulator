...
from src.storage.settings_repo import (
    get_setting, set_setting, parse_threshold, THRESHOLD_DEFAULTS,
...
class SettingsPage(QWidget):
    def __init__(self, conn: sqlite3.Connection, parent=None):
        ...
        debt_form.setSpacing(10)
        self.max_payoff_months_input = QLineEdit()
        self.max_payoff_months_input.setMaximumWidth(200)
        self.max_payoff_months_input.setPlaceholderText(str(DEFAULT_MAX_DEBT_PAYOFF_MONTHS))
        debt_form.addRow(
            "Max Debt Payoff (months):", self.max_payoff_months_input,
        )
        self.default_debt_rate_input = QLineEdit()
        self.default_debt_rate_input.setMaximumWidth(200)
        self.default_debt_rate_input.setPlaceholderText(
            f"{DEFAULT_DEBT_ANNUAL_RATE_PCT:g}"
        )
        debt_form.addRow(
            "Default Debt Annual Rate (%):", self.default_debt_rate_input,
        )
        layout.addLayout(debt_form)
    ...
    def refresh(self):
        ...
        self.max_payoff_months_input.setText(
            str(get_max_debt_payoff_months(self.conn))
        )
        self.default_debt_rate_input.setText(
            f"{get_default_debt_annual_rate_pct(self.conn):g}"
        )
        ...
        try:
            for key, inp in self.inputs.items():
                ...
                if key in THRESHOLD_KEYS:
                    ...
                    pct = float(cleaned)
                    # Thresholds are percentages; negatives flip the
                    # warning logic, > 100% is meaningless. Reject both
                    # explicitly here — `set_threshold` silently coerces
                    # to its default, which would leave the user thinking
                    # the value saved when it didn't.
                    if pct < 0 or pct > 100:
                        raise ValueError(
                            f"{key} must be between 0% and 100% (got {pct}%)."
                        )
                    ratio = pct / 100.0
                    raw = f"{ratio:.4f}".rstrip("0").rstrip(".")
                set_setting(self.conn, key, raw)
            payoff_text = self.max_payoff_months_input.text().strip()
            if payoff_text:
                payoff_val = int(float(payoff_text))
                if payoff_val <= 0:
                    raise ValueError(
                        "Max debt payoff months must be positive."
                    )
                set_max_debt_payoff_months(self.conn, payoff_val)
            rate_text = self.default_debt_rate_input.text().strip().rstrip("%").strip()
            if rate_text:
                rate_val = float(rate_text)
                if rate_val < 0:
                    raise ValueError(
                        "Default debt rate cannot be negative."
                    )
                set_default_debt_annual_rate_pct(self.conn, rate_val)
            self.status_label.setText("Settings saved.")
        except ValueError as e:
            QMessageBox.warning(self, "Error", str(e) or "Threshold values must be numbers.")
