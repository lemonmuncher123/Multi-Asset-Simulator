...
class DashboardPage(QWidget):
    def __init__(self, conn: sqlite3.Connection, parent=None):
        ...
        # Bankruptcy banner: shared widget so every page renders the same
        # styling/text. Existing tests reach for `bankruptcy_label`, so
        # alias the inner label.
        from src.gui.widgets.bankruptcy_banner import BankruptcyBanner
        self.bankruptcy_banner = BankruptcyBanner(self.conn)
        self.bankruptcy_label = self.bankruptcy_banner.label
        self.main_layout.addWidget(self.bankruptcy_banner)
        ...
        # --- Cashflow trend row: 12-month stacked bar showing what's
        # earning vs costing each month. Sits between the net-worth/mix
        # row above and the per-asset performance row below, completing
        # the "history → composition → cashflow → performance" reading
        # order.
        self.cashflow_row = QGridLayout()
        self.cashflow_row.setSpacing(12)
        self._cashflow_canvas = None
        self._cashflow_placeholder = QWidget()
        self._cashflow_placeholder.setStyleSheet("background: transparent;")
        self.main_layout.addLayout(self.cashflow_row)
        ...
        # --- Bankruptcy trigger detail (hidden unless there's an active
        # bankruptcy event). NOT a "list of overdue payments" — it's the
        # forensic record of what unfunded obligations triggered the
        # simulator to declare bankruptcy.
        self.bankruptcy_trigger_frame = QFrame()
        self.bankruptcy_trigger_frame.setObjectName("bankruptcy_trigger_frame")
        self.bankruptcy_trigger_frame.setStyleSheet(_SECTION_STYLE)
        bankruptcy_layout = QVBoxLayout(self.bankruptcy_trigger_frame)
        bankruptcy_layout.setSpacing(4)
        bankruptcy_layout.addWidget(
            _section_header("Bankruptcy Trigger — Unfunded Obligations"),
        )
        self.bankruptcy_trigger_table = make_table(
            ["Event Date", "Kind", "Asset", "Due Date", "Amount", "Notes"],
        )
        configure_expanding_table(self.bankruptcy_trigger_table)
        bankruptcy_layout.addWidget(self.bankruptcy_trigger_table)
        self.bankruptcy_trigger_frame.setVisible(False)
        self.main_layout.addWidget(self.bankruptcy_trigger_frame)
    ...
    def _apply_responsive_layout(self, width: int) -> None:
        ...
        try:
            ...
            self._reflow_cashflow_row(mode)
    ...
    def _reflow_cashflow_row(self, mode: str) -> None:
        cashflow_widget = self._cashflow_canvas or self._cashflow_placeholder
        # Full width — single column regardless of mode.
        self._populate_grid(self.cashflow_row, [cashflow_widget], 1)
    ...
    def _cleanup_figures(self):
        for attr in ('_trend_canvas', '_mix_canvas', '_drivers_canvas', '_cashflow_canvas'):
    ...
    def _swap_canvas(self, attr: str, placeholder_attr: str, fig, min_height: int = 260, stretch: int = 3):
        ...
        if attr == "_drivers_canvas":
            layout = self.perf_row
        elif attr == "_cashflow_canvas":
            layout = self.cashflow_row
        else:
            layout = self.charts_row
    ...
    def refresh(self):
        ...
        self._refresh_bankruptcy_banner(warnings)
        ...
        self._refresh_bankruptcy_trigger()
        self._refresh_activity()

    def _refresh_cashflow_chart(self):
        periods = compute_cashflow_series(self.conn, granularity="monthly")
        fig = create_cashflow_bar_figure(
            periods, granularity="monthly",
            compact=self._is_compact_chart_mode(),
        )
        self._swap_canvas("_cashflow_canvas", "_cashflow_placeholder", fig, 220, 1)

    def _refresh_bankruptcy_trigger(self):
        """Populate the Bankruptcy Trigger table from active bankruptcy
        events. Hidden when there are none. Surfaces only obligation-style
        triggers (debt/mortgage/multiple); negative_cash triggers are
        fully expressed by the bankruptcy banner alone.
        """
        events = [
            e for e in list_active_bankruptcy_events(self.conn)
            if e.trigger_kind in ("debt", "mortgage", "multiple")
        ]
        if not events:
            self.bankruptcy_trigger_frame.setVisible(False)
            self.bankruptcy_trigger_table.setRowCount(0)
            return
        self.bankruptcy_trigger_frame.setVisible(True)
        self.bankruptcy_trigger_table.setRowCount(len(events))
        for i, e in enumerate(events):
            asset = get_asset(self.conn, e.asset_id) if e.asset_id else None
            asset_name = (
                asset.name if asset
                else (f"asset #{e.asset_id}" if e.asset_id else "")
            )
            kind_label = e.trigger_kind.capitalize() if e.trigger_kind else ""
            self.bankruptcy_trigger_table.setItem(i, 0, QTableWidgetItem(e.event_date))
            self.bankruptcy_trigger_table.setItem(i, 1, QTableWidgetItem(kind_label))
            self.bankruptcy_trigger_table.setItem(i, 2, QTableWidgetItem(asset_name))
            self.bankruptcy_trigger_table.setItem(i, 3, QTableWidgetItem(e.due_date or ""))
            self.bankruptcy_trigger_table.setItem(
                i, 4, QTableWidgetItem(fmt_money(e.amount_due)),
            )
            self.bankruptcy_trigger_table.setItem(
                i, 5, QTableWidgetItem(e.notes or ""),
            )
        resize_table_to_contents(self.bankruptcy_trigger_table)

    def _refresh_bankruptcy_banner(self, warnings):
        # Delegated to the shared widget — same styling/text everywhere.
        # Pass the precomputed warnings so the banner doesn't run a second
        # scan; `refresh()` falls back to a fresh check if `warnings` is
        # None.
        self.bankruptcy_banner.refresh(warnings)
