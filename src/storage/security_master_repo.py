...
def bulk_upsert_securities(conn: sqlite3.Connection, records: list[SecurityMasterRecord]) -> int:
    if not records:
        return 0
    # Single executemany pays the per-statement prep cost once. The
    # prior per-row execute loop was the dominant cost on first launch
    # (thousands of seed securities); switching to executemany matches
    # the pattern already used by `price_repo.bulk_upsert_ohlcv`.
    rows = [
        (rec.symbol, rec.name, rec.asset_type, rec.exchange, rec.sector,
         rec.industry, rec.etf_category, int(rec.is_common_etf))
        for rec in records
