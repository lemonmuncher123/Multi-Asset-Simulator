...
def get_provider(asset_type: str, providers: dict[str, PriceProvider] | None = None) -> PriceProvider | None:
    # `providers is not None` (rather than truthiness) so an empty
    # dict means "explicit override with no providers" — used by
    # tests and offline flows to opt out of the live-yfinance
    # fallback. Truthiness check would fall through and silently
    # call yfinance.
    if providers is not None:
        return providers.get(asset_type)
