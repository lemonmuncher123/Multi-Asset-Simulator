...
from src.engines.portfolio import calc_cash_balance, calc_total_assets
...
def prepare_trade_preview(
    ...
    if is_amount_mode and not preview.blocking_errors:
        if draft.target_amount <= 0:
            ...
            derived_qty = math.floor(draft.target_amount / preview.trade_price)
            if derived_qty <= 0:
                preview.blocking_errors.append(
    ...
    if not preview.blocking_errors:
        ...
        preview.portfolio_value_before = calc_total_assets(conn)
        ...
        if draft.action == "buy":
            ...
            preview.portfolio_value_after = preview.portfolio_value_before
...
def _simulate_after_state(
    ...
    try:
        ...
        # instead of the user's customized thresholds.
        for table in ("properties", "debts", "settings"):
